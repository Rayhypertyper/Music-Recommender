...
def spotify_link(artist, track):
    ...
    #                 src= f'{item["external_urls"]["spotify"]}'
    #                 width="300" height="80" frameBorder="0"
    #                 allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
    #                 loading="lazy">
    #         </iframe>
    #         """
    # return st_html(iframe, height=100)


def recommend(a, song: str, artist: str, df: pd.DataFrame, model, text_pipeline, numerical_pipeline, text_features, numerical_features, feature_weights, top_n: int = 5):
...
def main():
    ...
    a = spotify_link(selected_artist, selected_track)
    num_rec = st.slider('Number of recommendations', 1, 10, 5)
    if st.button('Recommend'):
        recs = recommend(a, selected_track, selected_artist, df, model, text_pipeline, numerical_pipeline, text_features, numerical_features, feature_weights, top_n=num_rec)
        if recs:
